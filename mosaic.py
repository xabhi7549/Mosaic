...
# Change these 3 config parameters to suit your needs... done
...
class TargetImage:
    def __init__(self, image_path,day):
    ...
    try:
        ...
        for x in range(mosaic.x_tile_count):
            for y in range(mosaic.y_tile_count):
                ...
                small_box = (
                x * TILE_SIZE / TILE_BLOCK_SIZE, y * TILE_SIZE / TILE_BLOCK_SIZE, (x + 1) * TILE_SIZE / TILE_BLOCK_SIZE,
...
def mosaic(img_path, tiles_path,day):
    image_data = TargetImage(img_path,day).get_data()
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print('Usage: {} <image> <tiles directory> <Day>\r'.format(sys.argv[0]))
    else:
